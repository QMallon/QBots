...
#learnCodeByGaming's albion bot using it for referance on my diablo 2 bot
...
class Bot:
    ...
    IGNORE_RADIUS = 50
    TOOLTIP_MATCH_THRESHOLD = 0.3

    barFilter = HsvFilter(0, 235, 0, 179, 238, 255, 0, 0, 255, 0)
    botVision = None
    ...
    def __init__(self, window_offset, window_size):
        ...
        self.limestone_tooltip = cv.imread('filteredbar.png', cv.IMREAD_UNCHANGED)
    ...
    def confirm_tooltip(self, target_position):
        ...
        filterdtooltip = self.botVision.apply_hsv_filter(self.screenshot, self.barFilter)
        # check the current screenshot for the limestone tooltip using match template
        result = cv.matchTemplate(filterdtooltip, self.limestone_tooltip, cv.TM_CCOEFF_NORMED)
